...
const EXPECTED_TURNSTILE_HOSTNAME = "armeniankeyboard.com";
...
export async function POST(request: Request) {
  ...
  if (!validation.ok) {
    if (validation.reason === "honeypot") return htmlResponse("Thanks. You're on the newsletter.", 200);
  ...
  if (!turnstileOk) return htmlResponse("We could not verify this request. Please try again.", 400);
